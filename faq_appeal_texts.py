...
Placeholder: {nomor} {jam}
  - {nomor} : nomor telepon (tanpa +)
  - {jam}   : countdown format "HH:MM:SS" sesuai bukti screenshot
              (assets/form_v3_proof.jpg -> "Try again in 13:50:11")
...
2. SMS gak bisa diklik — cuma ada "Try again in HH:MM:SS"
...
kode". Kutip teks layar apa adanya ("Try again in HH:MM:SS").
...
_LONG_TEXTS = [
    ...
    (
        ...
        "dan di bawahnya cuma ada tulisan \"Try again in {jam}\" (format "
        "jam:menit:detik). Jadi WhatsApp gak mau kirim kode SMS ke nomor "
    ...
    (
        ...
        "\"Try again in {jam}\" (hitungan mundur jam:menit:detik). Cuma "
        "opsi Voice call yang kebuka, tapi pas saya coba juga gak ada "
        "telepon yang masuk.\n\n"
    ...
    (
        ...
        "nonaktif. Cuma ada teks kecil \"Try again in {jam}\" di "
        "sampingnya (hitungan jam:menit:detik), jadi saya gak bisa "
        "request kode via SMS sama sekali.\n\n"
    ...
    (
        ...
        "disabled dengan tulisan \"Try again in {jam}\" di bawahnya "
        "(hitungan mundur jam:menit:detik). Saya gak ngerti kenapa — "
        "saya gak salah masukin kode apa-apa karena kodenya emang gak "
        "pernah dikirim.\n\n"
    ...
    (
        ...
        "verify\", opsi SMS terkunci. Tulisannya \"Try again in {jam}\" "
        "(jam:menit:detik), jadi selama hitungan itu belum habis saya "
        "gak bisa minta kode SMS dan gak bisa daftar atau login sama "
        "sekali.\n\n"
    ...
    (
        ...
        "there's just a \"Try again in {jam}\" note under it (an "
        "hours:minutes:seconds countdown). So WhatsApp won't send me an "
        "SMS code until that timer runs out.\n\n"
    ...
    (
        ...
        "says \"Try again in {jam}\" (hours:minutes:seconds). The Voice "
        "call option is there, but when I tried it no call ever came "
        "through.\n\n"
    ...
    (
        ...
        "inactive. There's a small \"Try again in {jam}\" label next to "
        "it (hours:minutes:seconds countdown), so I can't request a code "
        "via SMS at all.\n\n"
    ...
    (
        ...
        "now. Every time I reach the \"Choose how to verify\" screen, "
        "the SMS option is always disabled with a \"Try again in {jam}\" "
        "note underneath (hours:minutes:seconds). I don't understand "
        "why — I never mistyped any code because a code was never sent "
        "in the first place.\n\n"
        "I tried the Voice call option once too, but no call arrived.\n\n"
        "SIM is active, signal is fine, texts from other apps come "
        "through normally. Please reset the countdown / unlock the SMS "
        "option for my number. Thanks."
    ),
    (
        "en", 1.2,
        "Hey WhatsApp,\n\n"
        "Verification issue on +{nomor}: on the \"Choose how to verify\" "
        "screen, the SMS option is locked. The label says \"Try again "
        "in {jam}\" (hours:minutes:seconds), which means for that whole "
        "countdown I can't request an SMS code and can't register or "
        "log in at all.\n\n"
...
def pick_long_appeal(nomor_clean: str, jam: str) -> str:
    """Pilih 1 teks berbobot, isi {nomor}/{jam}.

    jam: string countdown format "HH:MM:SS" (contoh "13:50:11").
         Kalau caller kirim angka jam saja, format ulang jadi "HH:00:00".
    """
    import random
    # Normalisasi: kalau caller kirim "24" atau "13", jadikan "13:00:00"
    j = str(jam).strip()
    if ":" not in j and j.isdigit():
        try:
            j = f"{int(j):02d}:00:00"
        except Exception:
            j = "13:50:11"
    ...
    return raw.replace("{nomor}", str(nomor_clean)).replace("{jam}", j)
